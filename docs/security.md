...
- **隧道服务方**：前端静态页由本机 Remote Bridge 同源伺服，不存在第三方静态托管。会话快照、事件与命令均在浏览器与本地扩展之间直接传输（经 Tunnelmole 隧道）；敏感参数位于 URL Fragment，不出现在隧道服务方请求日志中。
- **局域网**：本地 Bridge 只监听 `127.0.0.1` 随机端口，局域网设备必须经过隧道授权链路。
- **恶意网站**（跨站请求）：严格 Origin 白名单（隧道地址 + 本地开发地址，隧道地址在分享启动后动态加入），API 无 Origin 或非白名单一律 `403`；预检与响应只回显白名单 Origin。静态页面本身不做 Origin 校验（顶级导航无 Origin 头）。
...
- 用户消息、助手可见正文、思考过程（Thinking，截断至 50 000 字符）与助手错误信息（协议 v2 起的产品决策：完整转发所有消息）。
- 工具名/参数预览/结果（均截断）。
- Agent 运行状态、控制权状态、会话名与 cwd 尾段标签。

不接触：系统提示词、废弃分支、`custom`/`custom_message` 扩展私有数据、Session 文件路径与本机绝对路径。
