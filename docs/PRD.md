...
用户需要一种随时开启、无需部署专用中转服务器的临时分享方式。远端设备应能查看同一个 Pi Session 的活动分支（含思考过程与工具调用过程，即完整转发所有消息），并在获得授权后接管输入和中止能力；本机用户必须保留最终控制权，能够随时撤销远端权限。分享过程还必须避免公开系统提示词、废弃分支和扩展私有数据；前端由本机同源伺服，不经任何第三方托管，也不保存会话内容。
...
远端用户直接打开隧道分享链接：前端静态页由本机 HTTP Bridge 同源伺服，会话快照、增量事件和控制命令均在远端浏览器与本机扩展之间直接传输，不经过任何第三方服务端。由于 Tunnelmole 不适合承载入站 WebSocket 或持续 SSE，MVP 使用 HTTP 长轮询接收事件、使用 POST 发送命令，并通过单调递增 Cursor 支持断线恢复。
...
48. As a Pi 用户, I want to 让敏感分享参数位于 URL Fragment, so that 隧道服务方请求日志不会收到 Endpoint 和能力令牌。
49. As a Pi 用户, I want to 让前端由本机同源伺服, so that 会话正文不会经过任何第三方静态托管。
...
58. As a 自定义部署用户, I want to 用 PI_REMOTE_WEB_DIST 覆盖前端产物目录, so that 我可以使用自己构建的页面产物。
...
- Viewer URL 只包含 Viewer Capability；Controller QR 额外包含只能兑换一次的 Claim Capability。所有敏感连接参数位于隧道分享链接的 Fragment 中。
...
- 前端使用 React、TypeScript、Vite 和 PWA 构建能力，由本机 Remote Bridge 同源伺服（`web/dist`），不依赖任何静态托管服务。
...
- 前端产物缺省由本机同源伺服（`web/dist`），并允许通过 `PI_REMOTE_WEB_DIST` 覆盖产物目录。
...
- 建议按五个垂直阶段交付：本地 Session Bridge、授权与控制权、Tunnelmole 公网传输、本地同源前端、发布与安全加固。
